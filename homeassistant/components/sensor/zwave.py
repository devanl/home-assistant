...
def setup_platform(hass, config, add_devices, discovery_info=None):
    """Implementation of sensor platform to support set_indicator service."""
    def update(service):
        ...
        if entity_ids and value:
            ...
            for sensor_id in entity_ids:
                if sensor_id in SETTABLE_SENSORS:
                    SETTABLE_SENSORS[sensor_id].values.primary.data = \
                        int(value, 0)
                else:
                    _LOGGER.error("Could not find settable entity for %s",
                                  sensor_id)
        else:
...
class ZWaveIndicatorSensor(ZWaveSensor):
    """Representation of a Z-Wave sensor for COMMAND_CLASS_INDICATOR."""
